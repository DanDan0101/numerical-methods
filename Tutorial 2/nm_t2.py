...
def benchmark_M():
    Ms = 2**np.arange(1, 19)
    serial_times = np.zeros_like(Ms)
    parallel_times = np.zeros_like(Ms)
    speedups = np.zeros_like(Ms)

    for i, M in enumerate(Ms):
        N = 20
        print(f"Testing M={M}, N={N}")
        start_time = time.time()
        serial_computation(M, N)
        serial_time = time.time() - start_time
        start_time = time.time()
        parallel_computation(M, N)
        ...
        serial_times[i] = serial_time
        parallel_times[i] = parallel_time
        speedups[i] = speedup
    
    fig, ax = plt.subplots(dpi = 300, layout = "constrained")
    plt.plot(Ms, serial_times, label = "Serial")
    plt.plot(Ms, parallel_times, label = "Parallel")
    plt.xscale("log", base = 2)
    plt.yscale("log")
    plt.legend()
    plt.xlabel("Shots ($M$)")
    plt.ylabel("Wall time (s)")
    plt.title(f"Computation time for ${N}\\times{N}$ random matrices")
    plt.savefig(f"benchmark_N_{N}.png")
    plt.show()
    return np.array([Ms, serial_times, parallel_times, speedups])

def benchmark_N():
    Ns = 2**np.arange(8)
    serial_times = np.zeros_like(Ns)
    parallel_times = np.zeros_like(Ns)
    speedups = np.zeros_like(Ns)
    ...
    plt.ylabel("Wall time (s)")
    plt.title(f"Computation time for ${M}$ shots")
    plt.savefig(f"benchmark_M_{M}.png")
    plt.show()
    return np.array([Ns, serial_times, parallel_times, speedups])

def main():
    M = 2**12 # Number of random matrices
...
if __name__ == "__main__":
    m_times = benchmark_M()
    n_times = benchmark_N()
    np.save("benchmark_M.npy", m_times)
    np.save("benchmark_N.npy", n_times)