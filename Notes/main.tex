...
\end{align*}
Note that
\[\ddot{x}(t)=\dv{t}f(t,x(t))=\partial_tf+f\partial_xf,\]
...
\begin{itemize}
    ...
    \[u_{tt}-c^2u_{xx}=0.\]
    The wave equation is most compatible with initial-value conditions. Also, see the advection equation.
...
The idea of \textbf{ensemble samplers} is to run an ensemble of Metropolis-Hastings ``walkers'' in parallel. At each point, to update a point, we first choose a different helper point, then move along the line connecting those points according to some appropriate probability distribution. This avoids the oversampling tendencies of the single-walker Metropolis-Hastings algorithm, and leads to far quicker convergence. This update scheme is known as the ``stretch'' move.

The emcee library allows the choice of different move types (i.e. different proposal functions). For instance, the differential evolution (DE) move can improve the sampling for multimodal densities, which pose the problem about being stuck in some local region.

Since the emcee algorithms enjoy greater success for a large class of probability functions, it has more \textbf{affine invariance} compared to the traditional M-H algorithm, which requires tuning parameters to be chosen for each specific probability function. Additionally, emcee is much easier and faster, since it is parallelizable.
\subsection{FFT}
The \textbf{fast Fourier transform} is one of the most important algorithms of all time.
\begin{definition}[Discrete Fourier transform]
    The one-dimensional \textbf{discrete Fourier transform (DFT)} is defined as
    \[X_k=\sum_{m=0}^{n-1}x_m\exp(-2\pi ik\frac mn).\]
    It takes $n$ uniformly sampled, complex-valued inputs $x_m$ to $n$ uniformly spaced, complex-valued outputs $X_k$.
\end{definition}
We can write the DFT definition using an $n\times n$ complex-valued matrix $F$ with elements $F_{k,m}=\exp(-2\pi ik\flatfrac mn)$. Then, the DFT equation reads
\[X=Fx.\]
In particular, this is a symmetric matrix. One intuitive way to interpret the entries of the matrix is that row $k$ describes $n$ uniformly spaced samples from $k$ full oscillations of a sinusoid.

The FFT is used ubiquitously. Here are a few samples of FFT use cases:
\begin{itemize}
    \item Radio astronomy with CHIME and CHORD. Data preprocessing (filtering out RF interference) is done at the hardware level via an FPGA.
    \item It allows us to long-range interferometry, by applying different phases between two spatially-separated telescopes to adjust viewing angle.
    \item The STFT allows us to draw spectrograms, which allows us to study objects like fast radio bursts.
    \item Fourier transforms allow us to very efficiently compute convolutions, via the convolution theorem.
    \item DESI cosmology stuff for large-scale galaxy structure. In particular, working in Fourier space allows us to study baryon acoustic oscillations (BAOs).
\end{itemize}
The convolution theorem states that convolution becomes multiplication under the Fourier transform, and vice versa.
\begin{definition}[Discrete convolution]
    The convolution of a signal $x_i$ of size $N$, with a kernel $h_k$ of size $K$, is a result $y_i$ of size $N-K$,
...
\begin{definition}[Nyquist frequency]
    The \textbf{Nyquist frequency} is half of the sample rate, and it characterizes the highest frequency the Fourier transform can be defined up to, due to effects from aliasing.
\end{definition}
The interpretation of the Fourier transform as a matrix equation with square matrix $F$ lends itself nicely to an interpretation as a change of basis. The Fourier basis is an orthonormal basis. Another notable basis set is the Chebyshev polynomials.

The FFT (due to Cooley and Tukey) speeds up the computation from a naive $\order{n^2}$ (from the matrix multiplication point-of-view) to $\order{n\log{n}}$. The idea of FFT is divide-and-conquer (in an ``interlaced'' sort of way). In practice, the divide-and-conquer works best when the FFT is taken on a number of samples which is a product of small primes, such as a \textbf{power of $2$}.

For computational efficiency of the FFT, one thing done in practice is to pad data with $0$'s (or constant) until the number of samples is a power of $2$.

To see the interlacing in effect, the insight is as follows. Consider again the matrix equation
\[X_k=F_{km}x_m,\]
with
\[F_{km}=e^{-2\pi ikm/N}.\]
Now, suppose $k$ is in the first half of the range, i.e. $0\leq k<\frac N2$. Let $k'=k+\frac N2$. Then
\[F_{km}=e^{-2\pi ikm/N},\]
and it's not difficult to show that
\[F_{k'm}=e^{-\pi im}F_{km}=(-1)^mF_{km}.\]
The realization that allows us to do a divide-and-conquer algorithm is now that the terms with even $m$ are precisely the terms of a Fourier transform of length $\frac N2$. The terms with odd $m$ are similarly the terms of a Fourier transform of length $\frac N2$, but with signs introduced on for $F_{k'm}$, and up to a factor $e^{-2\pi ik/N}$.

In summary, for $0\leq k<\frac N2$, we can compute the Fourier transform by a divide-and-conquer recursion,
\begin{align*}
    X_k&=E_k+e^{-2\pi ik/N}O_k,\\
    X_{k+\frac N2}&=E_k-e^{-2\pi ik/N}O_k,
\end{align*}
where $E_k$ and $O_k$ are the Fourier transforms of size $\frac N2$, on the even and odd elements, respectively. Of course, once $N=1$, the Fourier transform is just trivial.
\subsection{Hydrodynamics}
Hydrodynamics is ubiquitous in physics. The basic equations of hydrodynamics are the \textbf{Euler equations}:
\begin{align*}
    \partial_t\rho+\div{\rho\vb{v}}&=0,\\
    \partial_t\rho\vb{v}+\div(\rho\vb{v}\otimes\vb{v}+\mathsf{I}P)&=\rho\vb g,\\
    \partial_t\qty(\frac12\rho v^2+\rho\eps)+\vb v\vdot\qty(\qty(\frac12\rho v^2+\rho\eps+P)\vb{v})&=\rho\vb v\vdot\vb g.
\end{align*}
The first equation is continuity. The second equation expresses conservation of momentum. The third equation expresses conservation of energy ($\eps$ is the specific internal energy). These should be supplemented with some equation of state, $P(\rho,\eps)$, for instance. We can make these equations more fancy by adding in effects of chemical reactions, self-interactions, thermal radiation, etc. For now, we consider the simple case of an ideal fluid with no external sources.

These equations all take the form
\[\boxed{\partial_tu+\div{\vb{f}(u)}=0},\]
for some flux $\vb{f}$. These are all local conservation equations. We can define macroscopic quantities,
\[U_V=\int_Vu\dd{V}.\]
These are conserved (up to a boundary term),
\begin{align*}
    \dv{t}U_V&=\int_V\partial_tu\dd{V}\\
    &=-\int_V\div{\vb f}\dd{V}\\
    &=-\oint_{\partial V}\vb f\vdot\dd{\vb{A}}.
\end{align*}
We now want a numerical scheme to solve (systems of) local conservation equations. We'll write this as
\[\partial_tu_i+\div{\vb{f_i}(\qty{u_j})}=0.\]
We can rewrite this as
\[\partial_tu_i+\pdv{\vb{f_i}}{u_j}\vdot\grad u_j=0.\]
We can diagonalize this matrix (for each $i$, I think?), and we will find
\[\partial_tr_i+\boldsymbol{\lambda_i}\vdot\grad r_i=0.\]
This motivates looking at the \textbf{advection equation}, which is a linear, hyperbolic, first-order PDE:
\[\boxed{\partial_tu+\lambda\partial_xu=0}.\]
Here, $\lambda$ is a constant. We can rewrite this in conservation form as
\[\partial_tu+\partial_x(\lambda u)=0.\]
This is a conservation law in 1D. Given an initial condition $u(t=0,x)=f(x)$, the solution to the advection equation is
\[u(t,x)=f(x-\lambda t).\]
Very simple. Now, let's discretize this, and put it on a grid. Define shorthand notation
\[u_j^n=u(t^n,x_j),\]
such that $t^n=n\Delta t$, and $x_j=j\Delta x$. Under an appropriate discretization scheme, we have
\[\frac{u^{n+1}_j-u^n_j}{\Delta t}+\lambda\frac{u^n_{j+1}-u^n_{j-1}}{2\Delta x}=0.\]
The diagram for the discretization is known as the \textbf{molecule} for the scheme. The update rule is
\[u_j^{n+1}=u_j^n-\frac{\lambda\Delta t}{2\Delta x}\qty(u_{j+1}^n-u_{j-1}^n).\]
This is known as the \textbf{Forward-Time Centered-Space (FTCS)} scheme. It is an explicit scheme, but it suffers from similar problems as the forward Euler scheme.

Let's do \textbf{von Neumann stability analysis}. To do this, take an ansatz,
\[u_j^n=\eps^ne^{ik\Delta xj}.\]
How does $\eps^n$ evolve? Since the advection equation is linear, this $\eps$ could model some additive noise. Then
\[\eps^{n+1}=\eps^n-\frac{\lambda\Delta t}{\Delta x}\frac{e^{ik\Delta x}-e^{-ik\Delta x}}{2}.\]
Now, define some amplification factor,
\[\abs{A}^2=\abs{\frac{\eps^{n+1}}{\eps^n}}^2=1+\qty(\frac{\lambda\Delta t}{\Delta x})^2\sin^2(k\Delta x)\geq1.\]
This is \textbf{unconditionally unstable}, which is bad.

What if we try a \textbf{Forward-Time Left-Space (FTLS)} scheme? In this scheme,
\[\partial_xu\to\frac{u_j^n-u_{j-1}^n}{\Delta x}.\]
If we repeat the same analysis, it turns out that
\[\abs{A}^2=1-4\frac{\lambda\Delta t}{\Delta x}\qty(1-\frac{\lambda\Delta t}{\Delta x})\sin^2(\frac{k\Delta x}{2}).\]
In terms of accuracy, the approximation of the first derivative is worse. But, the amplification factor looks like it could be less than $1$. The condition is that
\[\boxed{\frac{\Delta x}{\Delta t}>\lambda}.\]
This scheme is \textbf{conditionally stable}. This limit is known as the \textbf{Courant–Friedrichs–Lewy (CFL) condition}. The intuition has to deal with \textbf{upwinding}, in thinking about the causal structure of the spacetime. In practice, we choose some constant $c\in(0,1)$, then impose that
\[\Delta t=c\min\qty(\frac{\Delta x}{\lambda}).\]
Moving back to the original problem, we discretize 3D space into voxels. Then,
\begin{align*}
    U_{ijk}(\Delta t)&=U_{ijk}(0)+\sum_{\text{faces}}\int_0^{\Delta t}\dd{t}\int_{A_i}\dd{\vb{A_i}}\vdot\vb{f}\\
    &\approx U_{ijk}(0)+\Delta t\sum_{\text{faces}}\vb{f_i}^{\mathrm{num}}\vdot\Delta\vb{A_i}.
\end{align*}
In this case, the upwinding is much more complicated. We use this upwinding to compute the numerical approximation to $\vb{f_i}$. The update steps can then be done by forward Euler, Runge-Kutta, etc.

Often, we don't actually have $\vb{f}(u)$. Instead, we have
\[\partial_tu(q)+\div{\vb{f}(q)}=0.\]
The $q$'s are primitive, the $u$'s are conservative, and the $\vb f$ is flux. For things like advection, the primitive and conservatives are the same. For Newtonian hydrodynamics, they're easy to transform between each other. However, things are much more annoying for problems like relativistic hydrodynamics.

\section{Optimization}
Python is slow. If there exists a library function for something, don't do it yourself! In particular, avoid loops; prefer vectorized NumPy implementations.

One of the built-in profiling programs in Python is cProfile. Output files from cProfile can be visualized with the Python package flameprof.

Another profiler is the package line\_profiler.

Use these profilers to figure out what parts of your code is slow, before optimization!

File I/O could be done more efficiently by dumping binary instead of converting to strings, and by making use of buffers to write out data.
\newpage
\appendix
\section{Interview}
\subsection{Numerical Algorithms and Their Error Analysis}
\subsubsection{Root Finding}
\begin{itemize}
    \item \textbf{Bisection.} For 1D root-finding problems. Make two initial guesses with opposite sign. Refine guesses by bisecting the interval. Error bound goes as $\eps_n\leq 2^{-n}(b-a)$. Linear convergence.
    \item \textbf{Newton-Raphson.} For more general root-finding problems. Make one initial guess. Refine guesses by linear approximation,
    \[x_{n+1}=x_n-\frac{f(x_n)}{f'(x_n)}.\]
    Find error update by Taylor expanding the update rule,
    \[\eps_{n+1}\approx\frac12\frac{f''(x_*)}{f'(x_*)}\eps_n^2.\]
    Quadratic convergence.
\end{itemize}
\subsubsection{Riemann Sums}
Prototypical problem:
\[I=\int_0^1\dd{x}f(x).\]
1D definite integrals can be converted to the prototypical problem by compactification. Riemann sums can be defined from the left [or right],
\[I_0\approx\sum_{n=0}^{N-1}hf(x_{n[+1]}).\]
Analyze error for each step of the (left) sum by Taylor expanding
\[\eps_k=f(x_k)h-\int_{x_k}^{x_{k+1}}\dd{x}f(x).\]
Overall error is $\order{N^{-1}}$. Refinements use higher-order Taylor expansions (e.g. trapezoid rule, Simpson's rule), or expansions in different basis (e.g. Gaussian quadrature).
\subsubsection{ODEs}
Prototypical problem:
\[\dv{\vb{x}}{t}=\vb{f}(t,\vb{x}).\]
Typically given as an IVP. Higher-order coupled ODEs can be converted to first-order coupled ODE.
\begin{itemize}
    \item \textbf{Forward Euler.} Explicit update rule using first derivative,
    \[x_{n+1}=x_n+hf(t,x_n).\]
    Error from each step is from second derivative. Overall error is $\order{N^{-1}}$.
    \item \textbf{RK2 (midpoint rule).} Explicit update rule approximating slope at midpoint of interval,
    \[x_{n+1}=x_n+hf\qty(t_n+\frac h2,x_n+\frac h2f(t_n,x_n)).\]
    To estimate error, use lemma
    \[\dv{t}f(t,x(t))=\partial_tf+f\partial_xf.\]
    Thus, RK2 uses up to second derivative, and has error $\order{h^3}$ for each step, $\order{h^2}$ overall.
    \item \textbf{RK4.} Explicit update rule using weighted average (weights $[1,2,2,1]$) of following points:
    \begin{itemize}
        \item Starting point.
        \item Step to midpoint.
        \item Restep to midpoint.
        \item Step to end.
    \end{itemize}
    RK4 has error $\order{h^5}$ for each step, $\order{h^4}$ overall.
\end{itemize}
\subsubsection{Richardson Extrapolation}
Compute two updates, $x_{h_n}$ by stepping with step size $h_n$, and $x_{\flatfrac{h_n}2}$ by stepping twice with step size $\flatfrac{h_n}2$. Let the order of the update rule be $p$. Then,
\begin{align*}
    \eps_{h_n}&=ch^{p+1}+\dotsb,\\
    \eps_{\flatfrac{h_n}2}&\approx2^{-p}ch^{p+1}+\dotsb.
\end{align*}
This rearranges to
\begin{align*}
    \eps_{h_n}&\approx\frac{2^p}{2^p-1}(\eps_{h_n}-\eps_{\flatfrac{h_n}2}),\\
    \eps_{\flatfrac{h_n}2}&\approx\frac{1}{2^p-1}(\eps_{h_n}-\eps_{\flatfrac{h_n}2})\coloneq\eps_n.
\end{align*}
While $\eps_n>\eps_{\mathrm{tol}}$, estimate a new step size,
\[h^*_n\lesssim h_n\qty(\frac{\eps_{\mathrm{tol}}}{\eps_n})^{\frac{1}{p+1}}.\]
To update, set
\[x_{n+1}=x_{\flatfrac{h_n}2}-\eps_n,\]
and set
\[h_{n+1}=h^*_n.\]
\subsection{Monte Carlo}
\subsubsection{Bayesian Inference}
MC lets us gather samples/statistics from simulations. Examples of uses:
\begin{itemize}
    \item Particle physics simulations. Geant4, etc.
    \item Statistical physics.
    \item Astronomy.
\end{itemize}
Suppose we have parameters $\theta$ for our model, and we want to predict observations $\hat y_i$ given dependent variables $\hat x_i$. Then,
\[\hat y_i=f(\theta,\hat x_i).\]
Bayesian calculates the posterior as
\[p(\theta\mid\hat y_i,\hat x_i)=\mathcal L(\hat y_i\mid\hat x_i,\theta)\frac{p(\theta)}{p(\hat y_i)}.\]
We don't care about the evidence term $p(y_i)$, and we want an ``uninformative'' prior $p(\theta)$. Then we can sample the posterior, which is proportional to the likelihood,
\[p(\theta\mid\hat y_i,\hat x_i)\propto\mathcal L(\hat y_i\mid\hat x_i,\theta).\]
The fit can be done with MLE.
\subsubsection{Metropolis-Hastings}
Metropolis-Hastings is the simplest MCMC algorithm for sampling from a known distribution $p$. Set an initial configuration for the ``walker''. To update, sample a jump from a proposal distribution, $\vb{x'}\sim q(\vb{x'};\vb{x})$. Accept this proposal with probability
\[\min(1,\frac{p(\vb{x'})}{p(\vb{x})}\frac{q(\vb{x};\vb{x'})}{q(\vb{x'};\vb{x})}),\]
otherwise, don't jump. The history of walker positions converges in probability to the target distribution $p$.

To determine the number of steps to take, one can make plots of the trajectory through parameter space, comparing against the known distribution $p$. One estimate is the autocorrelation time $\tau$ (e.g. using acor).

Taking staggered steps through a high-dimensional parameter space is known as Gibbs sampling. It avoids the challenge of choosing a good multivariate joint probability distribution (unless the parameters are highly correlated, in which case the original problem returns), at the cost of possibly needing more steps.
\subsubsection{emcee}
The Python library emcee is an affine-invariant sampler for MCMC. It is an ensemble sampler, which runs an ensemble of Metropolis-Hastings ``walkers'' in parallel. The ``stretch'' update rule for each walker chooses a helper point, then draws a sample from the line connecting the walker and the helper point (in the affine space). The ``differential evolution'' update rule improves the sampling for multimodal densities. Emcee has several advantages:
\begin{itemize}
    \item Ensemble samplers have faster convergence.
    \item No need to tune parameters carefully for proposal distribution (affine invariance).
    \item Update rules avoid oversampling, work with multimodal densities, etc.
    \item Parallelizable, faster, easy to use.
\end{itemize}
\subsection{Fourier Transform}
\subsubsection{FFT}
The 1D DFT on $N$ points
\[X_k=\sum_{m=0}^{N-1}x_m\exp(-2\pi ik\frac mN).\]
Define a square matrix $F$, with entries
\[F_{k,m}=\exp(-2\pi ik\frac mN).\]
Then, the DFT can be written
\[X=Fx,\]
which has computational complexity $\order{N^2}$. In comparison, the FFT (Cooley-Tukey) is a divide-and-conquer algorithm. Suppose $N$ is a power of $2$. We use interlacing. Let $0\leq k<\frac N2$. Define $k'=k+\frac N2$. Then, it can be shown that
\[F_{k',m}=(-1)^mF_{k,m}.\]
Letting $E_k$ (resp. $O_k$) be the Fourier transform of size $\frac N2$ on the even (resp. odd) elements of $x$, the FFT then has
\begin{align*}
    X_k&=E_k+e^{-2\pi i\flatfrac kN}O_k,\\
    X_{k+\flatfrac N2}&=E_k-e^{-2\pi i\flatfrac kN}O_k.
\end{align*}
This algorithm has cost $\order{N\log{N}}$.
\subsubsection{Convolution Theorem}
The discrete convolution of two signals is
\[(x*k)_n=\sum_{m=-\infty}^\infty x_mk_{n-m},\]
with endpoint behavior dependent on the padding scheme. Naively, the computational cost is $\order{NK}$.

The convolution theorem gives a fast way to do circular convolution via the FFT,
\[(x_N*k)_n=\mathcal F^{-1}\qty{\mathcal F\qty{x}\cdot\mathcal F\qty{k}}.\]
Supposing $\log_2{N}>K$, the computational cost is then $\order{N\log{N}}$.
\subsection{Everything Else}
\subsubsection{Python Operations}
\begin{itemize}
    \item \textbf{Running time of a program.} Tqdm gives some general idea of the running time of loops. Small code snippets can be benchmarked with timeit. The whole program can be profiled with cProfile (built-in), and visualized with flameprof. Another profiler is line\_profiler.
    \item \textbf{Memory usage of a program.} Memory usage can be profiled with tracemalloc (built-in).
    \item \textbf{Array file I/O.} Use np.save or np.savetxt to write data. Use np.load or np.loadtxt to load data.
    \item \textbf{Checking documentation.} Check the online documentation. Alternatively, use the help() function. Another possibility is to look at the source code.
    \item \textbf{Checking data type.} Generically, use the type() function. For np.ndarray objects, use the dtype property.
\end{itemize}
\subsubsection{Reduction of Order}
A second-order ODE on configuration space can be turned into a first-order ODE on the tangent bundle.
\subsubsection{PDEs}
\begin{center}
    \begin{tabular}{c||c|c|c}
        Property & Elliptic & Parabolic & Hyperbolic \\
        \hline
        Prototypical problem & Laplace's equation & Heat (diffusion) equation & Wave equation \\
        Prototypical equation & $u_{xx}+u_{yy}=0$ & $u_t-Du_{xx}=0$ & $u_{tt}-c^2u_{xx}=0$ \\
        Domain of dependence & All & Past & Past light cone \\
        Domain of influence & All & Future & Future light cone \\
        Boundary conditions & Whole boundary & Initial state and endpoints & Initial data and endpoints
    \end{tabular}
\end{center}
Some notes:
\begin{itemize}
    \item The advection equation reads
    \[\pdv{\psi}{t}+\div(\psi\vb{u})=0.\]
    It expresses continuity. It is a first-order hyperbolic PDE.
    \item A general second-order homogeneous linear PDE on two variables reads
    \[Au_{xx}+Bu_{xy}+Cu_{yy}+Du_x+Eu_y+Fu=0.\]
    Interpreted as a conic, the sign of the discriminant $B^2-4AC$ is preserved under linear changes of variables, and determines the type of the PDE.
\end{itemize}
